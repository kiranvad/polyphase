...
def plot_energy_landscape(outdict):
    ...
    ax.plot_trisurf(grid[0, :], grid[1, :], energy, triangles=simplices, linewidth=0.2, antialiased=True,
...
def compute_chemical_potential(phi, m, chi):
    ...
    mu3 = 1 - phi[2] + m[2] * (-1 + chi[1] + chi[1] * phi[2] ** 2) + \
          m[2] * (phi[2] * (1 - 2 * chi[1] + phi[1] * (chi[0] + chi[1] - chi[2])) + phi[1] * (
                chi[0] * (phi[1] - 1) - chi[1] + chi[2])) + np.log(phi[2])
